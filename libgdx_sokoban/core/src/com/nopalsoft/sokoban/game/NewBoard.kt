...
class NewBoard() : Group() {
    ...
    val arrayOfCharacterMoves: GdxArray<Vector2> = GdxArray()

    // X previous position, Y current position.
    val arrayMovesBox: GdxArray<Vector2> = GdxArray()
    val arrayTiles: GdxArray<Tiles> = GdxArray(25 * 15)
    var moves = 0
    var time = 0f
                ...
                if (cell != null) {
                    ...
                    if (tile.properties != null) {
                        if (tile.properties.containsKey("tipo")) {
                            val tipo = tile.properties["tipo"].toString()
