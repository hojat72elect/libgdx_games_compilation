...
class WorldGameNew {
    ...
    private var arrayBodies: GdxArray<Body> = GdxArray()
    var worldBox = World(Vector2(0f, -9.8f), true)
    var coins = 0
    var maxDistance = 0
    var state = STATE_RUNNING
    var TIME_TO_CREATE_CLOUD = 15f
    var timeToCreateCloud = 0f
    ...
    var worldCreatedUpToY = player?.position?.y
    ...
    private fun createFloor() {
        ...
        val shape = EdgeShape()
        shape.set(0f, 0f, BasicScreen.WORLD_WIDTH, 0f)
    ...
    private fun createItem(y: Float) {
        val newItem = Pools.obtain(Item::class.java)
    ...
    private fun updatePlayer(
        ...
        if (Settings.numBullets > 0 && fire) {
            ...
            Settings.numBullets = Settings.numBullets - 1
        }
    }
